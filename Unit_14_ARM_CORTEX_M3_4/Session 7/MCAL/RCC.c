...
*
*/
uint32_t_ MCAL_RCC_SYSCLK(void){
...
*
*/
uint32_t_ MCAL_RCC_HCLK(void){

	/*
	* HPRE[3:0]: AHB prescaler
	*/
	uint32_t_ HCLK = (MCAL_RCC_SYSCLK() >> (g_AHB_Prescaler_Table[ (RCC->CFGR >> 4) & 0b1111 ]));
	return HCLK;
}
...
*
*/
uint32_t_ MCAL_RCC_PCLK1(void){

	/*
	* PPRE1[2:0]: APB Low-speed prescaler (APB1)
	*/

	uint32_t_ PCLK1 =  (MCAL_RCC_HCLK() >> (g_APB_Prescaler_Table[ (RCC->CFGR >> 8) & 0b111 ]));
	return PCLK1;
}
