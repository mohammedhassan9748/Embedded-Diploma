...
static void (*g_IRQ_CallBackPtr[3])(void);
...
void MCAL_UART_Init(UART_Config_t* UART_ConfigPtr){
	...
	*/
	uint32_t_ DIV_Mantissa = (uint16_t)(PCLKx/(16*UART_ConfigPtr->USART_BaudRate));
	uint32_t_ DIV_Mantissa_MUL100 = DIV_Mantissa*100;

	uint32_t_ DIV_Fraction_MUL100 = ((PCLKx*25)/(4*UART_ConfigPtr->USART_BaudRate)) - DIV_Mantissa_MUL100;
	uint32_t_  DIV_Fraction = (DIV_Fraction_MUL100*16)/100;
